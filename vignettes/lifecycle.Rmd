...
These changes make reduce the chance of new users coming across a deprecated function, but don't prevent those who already know about it from referring to the docs.
...
Using an expectation inside an `expect_snapshot()`[^2] is a convenient way to do this:
...
Alternatively, you can change the default value to `lifecycle::deprecated()` to make the deprecation status more obvious from the outside, and use `lifecycle::is_present()`[^3] to test whether or not the argument was provided.

[^3]: Unlike `missing()`, this works for both direct and indirect calls.

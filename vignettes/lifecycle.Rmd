...
The default development stage is usually ![Stable](figures/lifecycle-stable.svg){style="vertical-align:middle"}.
A function is considered stable when the author is happy with its interface and behaviour and major changes are unlikely.
Breaking changes will occur gradually, through a deprecation process.

The exception to the rule is sub-1.0.0 packages where all functions are considered ![Experimental](figures/lifecycle-experimental.svg){style="vertical-align:middle"}.
Experimental functions are made available so people can try them out and provide feedback, but the author makes no promises to avoid breaking changes.
All functions in sub-1.0.0 packages are experimental, but \>=1.0.0 packages can also contain some experimental functions.

(![Maturing](figures/lifecycle-maturing.svg){style="vertical-align:middle"} functions lie somewhere in between experimental and stable. It's hard to be precise about exactly what this means, so we are gradually phasing out our use of this badge.)

### End of life

There are two ways that a function might reach the end of its life: it might be superseded or deprecated.

-   A ![Superseded](figures/lifecycle-superseded.svg)[^2] function has a better alternative available, but the function itself is not going away. While a superseded function will not receive new features, it will be kept working for the foreseeable future so that existing code does not need to change.

...
When the development of a feature has reached completion and we are reasonably certain that its interface and behaviour are correct, it is promoted to _stable_. This is the probable end state of most functions.
...
1. <img src="figures/lifecycle-superseded.svg" alt = "Superseded" style="vertical-align:middle" /> A superseded feature is no longer under active development, and a known better alternative is available. However, it is indefinitely kept in the package for backward compatibility. The author only makes the necessary changes to ensure that the function continues working. No new features will be added, and only the most critical of bugs will be fixed.

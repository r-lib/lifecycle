...
lifecycle::deprecate_warn("1.0.0", "fun()", "newPackage::fun()")
...
Add a call to `deprecate_warn()` on the first line of the function:
...
If the deprecation isn't a simple replacement, use the `details` argument to provide your own message to the user:
...
It's good practice to test that you've correctly implemented the deprecation, testing that the deprecated function both still works and generates a useful warning.
Using an expectation inside an `expect_snapshot()`[^1] is a convenient way to do this:

[^1]: You can learn more about snapshot testing in `vignette("snapshot", package = "testthat")`.
...
If you have extensive existing tests for the deprecated function you can suppress the warning in those tests:
...
-   `deprecated_soft()` can be used before `deprecated_warn()`.
    ...
    This is a gentler form of deprecation that's useful when a package changes its extension interface --- it encourages package authors to update their code without affecting the end user.
...
To advertise that a function is experimental and the interface might change with deprecation, first add an experimental badge to the description:
...
And then in the body of the function call `signal_experimental()`:

```{r}
order_pizza <- function(size, flavour) {
  signal_experimental("0.1.0", "order_pizza()")
}
```

lifecycle doesn't currently provide tools to help users avoid experimental functions, but it likely will in the future.
...
Alternatively, you can change the default value to `lifecycle::deprecated()` to make the deprecation status more obvious from the outside, and use `lifecycle::is_present()`[^2] to test whether or not the argument was provided.

[^2]: Unlike `missing()`, this works for both direct and indirect calls.

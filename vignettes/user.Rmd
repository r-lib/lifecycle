...
You'll notice that the warning only appears the first time we call it --- lifecycle only notifies you every 8 hours to avoid disrupting your work if you've used a deprecated function in many places.
So how do you track down exactly where the warning came from?
lifecycle provides two main tools.
...
The plural variant `lifecycle::last_warnings()` shows backtraces for all the warnings that were signalled in the last command. If you're a package developer this is especially helpful in conjunction with `devtools::test()`.
